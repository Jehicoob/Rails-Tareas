['juan', 'andrea', 'leon', 'andres', 'natalia', 'camilo', 'rusbel', 'jehicoob'].each do |name|
...
owner = User.find_by(email: 'jehicoob@test.com')
[
  ...
  participants = participant_set.map do |participant|
    ...
    Participant.new(
      user: User.find_by(email: "#{user_name}@test.com"),
      role: role.to_i
    )
  end
  Task.create!(
